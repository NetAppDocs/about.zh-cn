...
NOTE: 在 Beta 阶段，Doc 使用 docs.netapp.com 上发布的大部分（但不是全部）内容来回答问题。 Doc 熟悉所有ONTAP、 NetApp Console、 StorageGRID、E-Series、 Keystone和SnapCenter文档，并使用它们来回答有关这些产品的问题。另一方面，Doc 不熟悉一些遗留产品的文档。例如，Doc 不使用SnapDrive for UNIX文档来提供答案。在整个 Beta 测试过程中，我们致力于完善 Doc 的知识。如果您需要 Doc 没有的信息，请发邮件至：ng-doccomments@netapp.com[让我们知道]。
...
====
--
询问完成任务的步骤。:: 要求 Doc 包含完成流程的步骤。不要问“如何在没有互联网的情况下安装NetApp控制台代理？”，而是尝试：
+
--
[]
====
在没有互联网的情况下安装NetApp控制台代理应该遵循哪些步骤？
...
+
--
例如，不要询问 Doc _如何在NetApp控制台中配置勒索软件保护和备份计划？_ 而是尝试：

[]
====
在NetApp控制台中配置勒索软件保护的步骤是什么？
